...
checkPassword = async (username, plaintextPassword) => {
    ...
    if (globalStore[username]) {
        let result = await bcrypt(plaintextPassword, globalStore[username])
        ...
        if (result) {
            ...
            console.log(`Welcome, ${username}, your signed in user`)
        }
        else {
...
hashPassword = async (username, password) => {
    ...
    console.log(`User ${username} is created. Password is ${hashedPassword}`)
}
